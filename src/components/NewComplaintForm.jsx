...
const fetchDropdownOptions = async () => {
  try {
    ...
    if (data && data.table && data.table.rows) {
      ...
      // NEW: Technician mapping array to store name-contact pairs
      ...
      data.table.rows.forEach((row, index) => {
        if (row.c) {
          ...
          // Column E & F: Technician Name & Contact Mapping
          const techName = row.c[4] && row.c[4].v !== null && row.c[4].v !== "" && row.c[4].v !== "Technician Name" ? row.c[4].v : null
          const techContact = row.c[5] && row.c[5].v !== null && row.c[5].v !== "" && row.c[5].v !== "Technician Contact" ? row.c[5].v : null
      ...
      // NEW: Store technician mapping for auto-fill functionality
    ...
    // Fallback options में भी mapping add करें
